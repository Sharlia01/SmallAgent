...
def test_rag_tool_returns_standard_result_and_preserves_sources(monkeypatch):
    ...
    monkeypatch.setattr(rag_search, "retrieve_content", fake_retrieve_content)
    ...
    assert result.metadata == {"result_count": 1, "top_k": 8}
...
def test_rag_tool_converts_retrieval_exception_into_failure(monkeypatch):
    ...
    monkeypatch.setattr(rag_search, "retrieve_content", fail_retrieval)
